...
b = (function() local result = set {} for _, i in a do result.add(((mod_operator(i, 2)) == 1) and i or 0) end return result end)()
assert((len(b) == 3))
assert((operator_in(0, b)))
assert((operator_in(1, b)))
assert((operator_in(3, b)))
a = set {_to_null(0, 1, 2, 3)}
b = (function() local result = set {} for _, i in a do if ((mod_operator(i, 2)) == 1) then result.add(i) end end return result end)()
assert((len(b) == 2))
assert((operator_in(1, b)))
assert((operator_in(3, b)))
a = set {_to_null(0, 1)}
