...
e = (function() local result = dict {} for _, k, v in d.items() do result[k] = ((mod_operator(k, 2)) == 1) and v or 10 end return result end)()
assert((e[_to_null(0)] == 10))
assert((e[_to_null(1)] == 1))
assert((e[_to_null(2)] == 10))
f = (function() local result = dict {} for _, k, v in d.items() do result[(v == 0) and k or (v + 1)] = ((mod_operator(k, 2)) == 1) and v or 10 end return result end)()
...
d = dict {[_to_null(0)] = _to_null(dict {[_to_null("a")] = _to_null(1), [_to_null("b")] = _to_null(2)}), [_to_null(1)] = _to_null(dict {[_to_null("c")] = _to_null(3), [_to_null("d")] = _to_null(4)})}
e = (function() local result = dict {} for _, k, v in d.items() do if ((mod_operator(k, 2)) == 0) then for _, nk, nv in v.items() do if (nv > 1) then result[nk] = ((mod_operator(nv, 2)) == 1) and nv or (nv + 1) end end end end return result end)()
